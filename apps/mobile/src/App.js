...
import SupplierAiCoach from "./SupplierAiCoach";
...
function SupplierHome({ onLogout }) {
  ...
  const [form, setForm] = useState({ name: "", category: "", description: "", price_fcfa: "", old_price_fcfa: "", stock: "", sku: "", image_url: "" });
  const [saving, setSaving] = useState(false);

  const resetForm = () => { setForm({ name: "", category: "", description: "", price_fcfa: "", old_price_fcfa: "", stock: "", sku: "", image_url: "" }); setEditingId(null); setShowAdd(false); };
  ...
  const startEdit = product => {
    ...
    setShowAdd(true);
    setForm({ name: product.name || "", category: product.category || "", description: product.description || "", price_fcfa: String(product.price_fcfa ?? ""), old_price_fcfa: product.old_price_fcfa == null ? "" : String(product.old_price_fcfa), stock: String(product.stock ?? ""), sku: product.sku || "", image_url: product.image_url || "" });
  };

  const submitProduct = async () => {
    setSaving(true); setError("");
    try {
      const payload = { ...form, price_fcfa: Number(form.price_fcfa), old_price_fcfa: form.old_price_fcfa === "" ? null : Number(form.old_price_fcfa), stock: Number(form.stock || 0) };
      if (editingId) await supplierApi.updateProduct(editingId, payload);
      else await supplierApi.createProduct(payload);
      resetForm(); await load();
    } catch (e) { setError(e.message || "Impossible d'enregistrer le produit."); } finally { setSaving(false); }
  };

  const deactivate = async id => { setError(""); try { await supplierApi.deactivateProduct(id); await load(); } catch (e) { setError(e.message || "Impossible de désactiver le produit."); } };

  const supplierUniverse = me?.universe || me?.business_type || me?.space || me?.category || "marketplace";
  const normalizedUniverse = String(supplierUniverse).toLowerCase();
  const universe = normalizedUniverse.includes("food") || normalizedUniverse.includes("saveur") || normalizedUniverse.includes("culin") ? "food" : normalizedUniverse.includes("travel") || normalizedUniverse.includes("évasion") || normalizedUniverse.includes("evasion") || normalizedUniverse.includes("tour") ? "travel" : "marketplace";

  return <SupplierAiCoach universe={universe}>
    <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.page}><Header title={me?.business_name || "Espace fournisseur"} subtitle="Mon activité" onLogout={onLogout}/><View style={styles.statGrid}><Stat label="Produits actifs" value={stats?.active_products ?? "—"}/><Stat label="En validation" value={stats?.pending_products ?? "—"}/><Stat label="Stock total" value={stats?.total_stock ?? "—"}/></View>{error ? <Text style={styles.error}>{error}</Text> : null}<View style={styles.toolbar}><Text style={styles.sectionTitle}>Mes produits</Text><Button title={showAdd ? "Fermer" : "+ Ajouter"} secondary onPress={() => { if (showAdd) resetForm(); else setShowAdd(true); }}/></View>{showAdd ? <View style={styles.card}><Text style={styles.cardTitle}>{editingId ? "Modifier le produit" : "Nouveau produit"}</Text><TextInput placeholder="Nom" placeholderTextColor="#9ca3af" value={form.name} onChangeText={v => setForm({ ...form, name: v })} style={styles.input}/><TextInput placeholder="Catégorie" placeholderTextColor="#9ca3af" value={form.category} onChangeText={v => setForm({ ...form, category: v })} style={styles.input}/><TextInput placeholder="Description" placeholderTextColor="#9ca3af" value={form.description} onChangeText={v => setForm({ ...form, description: v })} style={[styles.input, styles.textarea]} multiline/><TextInput placeholder="Prix en FCFA" placeholderTextColor="#9ca3af" keyboardType="numeric" value={form.price_fcfa} onChangeText={v => setForm({ ...form, price_fcfa: v })} style={styles.input}/><TextInput placeholder="Ancien prix (optionnel)" placeholderTextColor="#9ca3af" keyboardType="numeric" value={form.old_price_fcfa} onChangeText={v => setForm({ ...form, old_price_fcfa: v })} style={styles.input}/><TextInput placeholder="Stock" placeholderTextColor="#9ca3af" keyboardType="numeric" value={form.stock} onChangeText={v => setForm({ ...form, stock: v })} style={styles.input}/><TextInput placeholder="SKU (optionnel)" placeholderTextColor="#9ca3af" autoCapitalize="characters" value={form.sku} onChangeText={v => setForm({ ...form, sku: v })} style={styles.input}/><TextInput placeholder="URL de l'image" placeholderTextColor="#9ca3af" autoCapitalize="none" value={form.image_url} onChangeText={v => setForm({ ...form, image_url: v })} style={styles.input}/><Button title={saving ? "Enregistrement…" : editingId ? "Enregistrer les modifications" : "Soumettre à Egonar"} onPress={submitProduct} disabled={saving || !form.name || !form.category || !form.price_fcfa}/>{editingId ? <Button title="Annuler" secondary onPress={resetForm} disabled={saving}/> : null}</View> : null}{products.slice(0, 20).map(p => <View style={styles.row} key={p.id}><View style={styles.rowMain}><Text style={styles.rowTitle}>{p.name}</Text><Text style={styles.rowMeta}>{p.price_fcfa} FCFA · stock {p.stock} · {p.approval_status}</Text><Text style={[styles.status, p.approval_status === "PENDING" ? { color: colors.warn } : p.approval_status === "REJECTED" ? { color: colors.danger } : { color: colors.ok }]}>{p.approval_status === "PENDING" ? "EN VALIDATION" : p.approval_status === "APPROVED" ? "APPROUVÉ" : "REFUSÉ"}</Text></View><View style={styles.rowActions}><Button title="Modifier" secondary onPress={() => startEdit(p)} />{p.active ? <Button title="Désactiver" danger onPress={() => deactivate(p.id)} /> : <Text style={[styles.status, { color: colors.danger }]}>INACTIF</Text>}</View></View>)}<Button title="Actualiser" secondary onPress={load}/></ScrollView></SafeAreaView>
  </SupplierAiCoach>;
}

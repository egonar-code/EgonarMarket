...
import SupplierUniverseGate from "./SupplierUniverseGate";
...
function SupplierHome({ onLogout, universe }) {
  ...
  const spaceName = { MARKET: "Market", SAVEURS: "Saveurs", EVASION: "Évasion" }[universe] || "Market";
  ...
  const submitProduct = async () => {
    ...
    try {
      const payload = { ...form, price_fcfa: Number(form.price_fcfa), old_price_fcfa: form.old_price_fcfa === "" ? null : Number(form.old_price_fcfa), stock: Number(form.stock || 0), universe };
...
export default function App() {
  ...
  if (MOBILE_ROLE === "admin") return <AdminHome onLogout={logout}/>;
  return <SupplierUniverseGate>{({ universe }) => <SupplierHome universe={universe} onLogout={logout}/>}</SupplierUniverseGate>;
}

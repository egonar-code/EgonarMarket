...
export async function api(path, options = {}, baseUrl = API_BASE_URL) {
  ...
  if (!response.ok) { const error = new Error(data?.error || `Erreur API (${response.status})`); error.status = response.status; error.data = data; throw error; }
  return data;
}

export const supplierApi = {
  login: async (email, password) => { const data = await api("/api/supplier/login", { method: "POST", body: JSON.stringify({ email, password }) }); await setSessionToken(data.token); return data; },
...
export const adminApi = {
  login: async (email, password) => { const data = await api("/api/admin/login", { method: "POST", body: JSON.stringify({ email, password })); await setSessionToken(data.token); return data; },
  me: () => api("/api/admin/me"),

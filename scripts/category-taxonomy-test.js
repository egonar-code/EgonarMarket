...
const file = path.join(__dirname, '..', 'db', 'migrations', '20260916_category_taxonomy.sql');
...
for (const [universe, categories] of Object.entries(required)) {
  ...
  for (const category of categories) {
    if (!sql.includes(`'${universe}','${category}`) && !sql.includes(`'${universe}',\n'${category}`)) {
      throw new Error(`Catégorie manquante: ${universe} / ${category}`);
    }
  }
}
